  ...
  const handleFavorite = (e, song) => {
    if (playlist.includes(song.track_id)) {
      ...
      fetch("/api/user/remove-from-library", {
        ...
        body: JSON.stringify({id: song.track_id})
      })
       .then(res => res.json())
       .then(data => {
          if (data.success) {
           ...
           // change state for playlist
            ...
            setLibrary([...library, 
              { track_id: song.track_id, album_art_path: song.album_art_path, album_id: song.album_id,  
                ...
                album_name: song.album_name, duration: song.duration, plays: song.plays 
              }]) // library is an array of objects
         }
       })
       .catch(error => {
         console.log(error);
    ...
    return (
      ...
      <div className={styles.song}>
        ...
        <div className={styles.callToAction} >
          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class={props.favorite ? styles.favorite : ''} viewBox="0 0 16 16" onClick={(e) => handleFavorite(e, props)}>
